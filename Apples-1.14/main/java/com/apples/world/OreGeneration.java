...
public class OreGeneration {
	...
	public static void load() {
		...
		if(Config.Holder.bedrockGeneration) {
			...
			//oreGeneration(new OreFeatureConfig(APPLES_BEDROCK, BlockLoader.BEDROCK_ORE.get().getDefaultState(), 4), 1, 1, 4, 8);
	...
	private static void oreGeneration(OreFeatureConfig orefeatureconfig, int count, int bottomoffset, int topoffset, int max) {
		...
		ConfiguredFeature<?> configuredfeature = Biome.createDecoratedFeature(Feature.ORE, orefeatureconfig, Placement.COUNT_RANGE, new CountRangeConfig(count, bottomoffset, topoffset, max));
