			...
			if(!world.isRemote) {
		        ...
		        if(list.size() <= 10 && livingattackevent.getSource() instanceof EntityDamageSource) {
		        	...
		        	if(livingattackevent.getSource().getTrueSource() != null && world.getDifficulty() != Difficulty.PEACEFUL) {
		                ...
		                int j = MathHelper.floor(livingentity.posY);
		                ...
		                for(int l = 0; l < 50; ++l) {
		                    ...
		                    if (world.getBlockState(blockpos).func_215682_a(world, blockpos, zombieentity)) {
		                    	
		                        zombieentity.setPosition((double)i1, (double)j1, (double)k1);
		                        
		                        if (!world.isPlayerWithin((double)i1, (double)j1, (double)k1, 7.0D) && world.checkNoEntityCollision(zombieentity) && world.areCollisionShapesEmpty(zombieentity) && !world.containsAnyLiquid(zombieentity.getBoundingBox())) {
